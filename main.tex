...
\begin{abstract}
    % 随着硬件设计需求越来越复杂和开源硬件生态的发展，Chisel 和 RISC-V 逐渐流行。
    ...
    % Chisel 作为 Scala 语言的领域特定语言，借助高级语言的特性，逐渐成为硬件设计开发的选择趋势。
    ...
    % 尤其是 RISC-V 开源指令集生态中，许多有代表性的项目均采用 Chisel 实现。
    ...
    % 然而目前对于 RISC-V 设计正确性的主流框架是采用 RISC-V Formal 框架，其是在 SystemVerlog 层次进行验证的。
...
% 这是一个正在进行的项目，目前支持 RV32/64IMC 指令集。
...
% 首先， RISC-V 实现的核心以及其可能连接的内存和 外部 IO，都作为 |rvfi_wrapper| 模块的组成。
...
% 同时在核心中，需要实现 RVFI 接口，输出在指令执行过程中的一系列状态信息。
...
% 例如对于 |add| 指令验证的 checker 模块中，会实例化一个对应的 |insn_spec| 模块。
% 在该模块中，会依据官方规范来定义对于加法指令，会对寄存器等状态变量产生怎样的影响。
...
% 然而 RISC-V Formal 框架有一些局限性。
However, RISC-V Formal has some limitations. 
% 首先在 RISC-V Formal 中，由于底层使用 SVA 来定义性质，因此整个框架只适用于 SystemVerilog 实现的 RISC-V 处理器。
...
% 同时， Chisel 生成的 SystemVerilog 代码可读性很差而且变量名称均有变化。
In addition, the SystemVerilog code generated by Chisel is unreadable, and the variable names are all changed.
% 这使得，无论是在生成的 SystemVerilog 中增加 RVFI 接口，或者是在 Chisel 层次定义接口再对生成的 SystemVerilog 使用 RISC-V Formal 都是不可行的。
...
% 在本文中，我们提出了 RVFC ，一个在 Chisel 层次上对 RISC-V 处理器进行形式化验证的框架。
In this paper, we introduce RVFC, a framework for formal verification of RISC-V processors at the Chisel level.
% RVFC 提供了一套在 Chisel 层次上类似于 RISC-V Fomal 框架的验证流程，这是已经经过工业实践证明有效的验证流程。
...
% 同时 RVFC 借助模块化的设计，从而支持更加自由的验证内容配置和算法引擎选择。
...
% 在之后的工作中，我们将会找一些更加复杂的 Chisel 层次实现的 RISC-V 处理器，使用 RVFC 框架进行验证。
...
% 我们目前仍然只能验证一些 demo，尚未具备工业应用的能力，这需要更多复杂的实例验证来不断完善 RVFC 框架。
...
% 同时我们还在继续开展验证算法的优化工作，目前的验证算法在应对大规模电路验证中往往会难以满足需求，我们希望改进现有的算法来提高验证效率。
